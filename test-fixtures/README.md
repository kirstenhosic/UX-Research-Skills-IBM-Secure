...
"Senior SRE at Contoso Financial" line is the one for safety. At
...
- **"Senior SRE at Contoso Financial"** is **allowed** at both internal tiers and must not be flagged. Role plus account is a category of user at a category of customer, and "Senior" is a level, not a unique position. A gate that flags this is over-triggering, and this line is the main control for that. It blocks at `external`, so flipping the fixture's declared destination should flip the verdict. If the plan's consent terms promised full anonymization, it blocks at every tier — consent wins. This one line is the destination-tier test; change the declared destination and the verdict should change with it.
